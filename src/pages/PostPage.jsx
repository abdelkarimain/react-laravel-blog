...
const PostPage = () => {
  ...
  useEffect(() => {
    try {
      const fetchRecentPosts = async () => {
        const res = await fetch(`/api/posts/recent/${postslug}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${auth_token}`,
          },
  ...
  return (
    <main className="p-3 flex flex-col max-w-6xl mx-auto min-h-screen">
      ...
      <Link to={`/categories`} className="self-center mt-5">
      ...
      <div className="flex justify-between p-3 border-b border-slate-500 mx-auto w-full max-w-5xl text-xs">
        ...
        <span className="italic">{post && readingTime(post.content)} mins</span>
      </div>
      <div
      ...
      <div className="mx-auto w-full max-w-2xl lg:max-w-7xl my-10">
        <div className="flex flex-wrap items-center justify-between gap-x-8 gap-y-3">
          <h2 className="text-3xl font-medium sm:text-3xl lg:text-[30px]">
            Comments
          </h2>
        </div>
      ...
      {relatedPosts?.length > 0 && (
        <div className="mx-auto w-full max-w-2xl px-6 lg:max-w-7xl my-10">
          <div className="flex flex-wrap items-center justify-between gap-x-8 gap-y-3">
            ...
            <Link
              className="inline-flex rounded-sm transition duration-300 leading-none focus:outline-none focus-visible:ring-2"
              to={`/all-posts`}
            >
              View all →
            </Link>
