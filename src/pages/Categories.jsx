    ...
    <main className="flex justify-center px-7 py-16 text-white dark:bg-gradient-to-br from-gray-800 to-gray-900 grow">
      <div className="container">
        <div className='flex flex-col gap-6 p-28 px-3 max-w-6xl mx-auto'>
          <h1 className='text-2xl font-bold text-slate-900 lg:text-6xl dark:text-gray-200 text-center md:text-start'>All Categories</h1>
          <p className='text-gray-800 dark:text-gray-300 text-xl text-center md:text-start'>
            These are all our Categories. More coming soon.🔍🔍
          </p>
        </div>
        <ul className="space-y-16 px-3">
                ...
                {categoryPosts.posts.map((post) => (
                  <li key={post.id} className=''>
                    <Link to={`/post/${post.slug}`} className="mb-2
                     text-gray-900 dark:text-gray-100 flex items-center p-2 dark:bg-gray-600 dark:border-t dark:border-gray-600 shadow-lg group rounded-xl shadow-primary-800/10">
                      <div className=''>
                        <h3 className="font-medium capitalize cat-post-content">{post.title}</h3>
                        <p className="cat-post-content text-gray-900 text-xs
                         group-hover:text-yellow-500 dark:text-gray-300 break-all">
