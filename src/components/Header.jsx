...
const Header = () => {
  ...
  const updateInput = async (input) => {
    const filtered = postsListDefault.filter((post) => {
      return (
        post.title.toLowerCase().includes(input.toLowerCase().trim()) ||
    ...
    <>
      <Navbar className="border-b-2 dark:bg-gradient-to-b from-gray-800 to-gray-900 bg-slate-100 text-gray-700">
        ...
        <div className="flex gap-2 md:order-2">
          <Button
            className="bg-gray-200 border-2 border-gray-300 cursor-pointer me-7"
            pill
            color="gray"
            onClick={() => {
          ...
          >
            {showSearchModal ? (
              <BiX className="text-xl dark:text-white text-slate-800" />
            ) : (
          ...
          <div className="hidden md:flex gap-2">
            <Dropdown label={t("language")} size="sm" color="gray">
              {languages.map((lang) => (
                <Dropdown.Item
                  key={lang.code}
                  onClick={() => i18next.changeLanguage(lang.code)}
                  disabled={lang.code === currentLangCode}
                  style={{ opacity: lang.code === currentLangCode ? 0.5 : 1 }}
                >
                  {lang.name}
                </Dropdown.Item>
              ))}
          ...
          {currentUser ? (
            ...
            >
              ...
              <Dropdown.Divider />
              <Dropdown.Item onClick={() => dispatch(logoutSuccess())}>
                {t("signOut") || "Sign Out"}
              </Dropdown.Item>
            </Dropdown>
          ) : (
        ...
        <Navbar.Collapse as={"div"}>
          ...
          <Navbar.Link className="md:hidden" as={"div"}>
            ...
            >
              {theme === "light" ? (
                <>
                  <FaMoon className="text-lg me-1" /> {t("dark") || "Dark"}
                </>
              ) : (
                <>
                  <FaSun className="text-lg me-1" /> {t("light") || "Light"}
                </>
              )}
            </Button>
          </Navbar.Link>
        ...
        >
          <div className="container mx-auto py-4 text-black">
            <input
              ...
              autoFocus
              className="dark:bg-gray-700 dark:text-gray-50 w-full text-grey-800 transition focus:outline-none focus:border-transparent px-3 appearance-none leading-normal text-xl lg:text-2xl"
            />
          </div>
          <div>
            {input && postsList && postsList.length > 0 ? (
              postsList.map((post) => (
                <p
                  className="container mx-auto text-black border-b-4 px-4"
                  key={post.id}
                >
                  <Link
                    to={"/post/" + post.slug}
                  ...
                  >
                    ...
                    <span className="font-bold break-all whitespace-pre-wrap">
                      {post.title}
                      <br />
                      <span className="text-sm bg-slate-400 px-2 py-1">
                        {post.category}
                      </span>
                    </span>
                  </Link>
              ...
              <p className="container mx-auto border-b-4 px-4">
                <span className="font-bold">{t("noResults") || "No results found"}</span>
              </p>
            )}
          </div>
        </div>
